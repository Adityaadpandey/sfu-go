...
import { ConnectionQuality, SignalingMessage } from '../types';
...
export const useWebRTC = () => {
      ...
      try {
        ...
        if (outboundStats.length > 0) {
          ...
          if (settings.autoQuality && level === 'poor') {
            ...
            log(
              `Poor connection detected (${lossRate.toFixed(
                1
              )}% loss), consider switching to lower quality`,
              'warning'
            );
          }
        }
  ...
  const handleSignalingMessage = useCallback(
    async (msg: SignalingMessage | Record<string, unknown>) => {
      switch (msg.type) {
        case 'ping':
          if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
            wsRef.current.send(
        ...
        case 'renegotiate':
          const reason =
            msg.data && typeof msg.data === 'object' && 'reason' in msg.data
              ? (msg.data as { reason: string }).reason
              : 'unknown';
          const trackCount =
            msg.data && typeof msg.data === 'object' && 'trackCount' in msg.data
        ...
        case 'quality-stats':
          if (msg.data && typeof msg.data === 'object') {
            ...
            if (
              qualityData.peerId &&
              qualityData.level &&
              typeof qualityData.packetLoss === 'number'
            ) {
              ...
              if (
                ...
                qualityData.peerId === peerIdRef.current &&
                qualityData.level === 'poor'
              ) {
                ...
                Object.entries(layersByTrack).forEach(([trackId, layers]) => {
                  if (layers.includes('l')) {
                    sendSignalingMessage({
                      type: 'layer-switch',
                      data: { trackId, targetRid: 'l' },
                    });
                  } else if (layers.includes('m')) {
                    sendSignalingMessage({
                      type: 'layer-switch',
                      data: { trackId, targetRid: 'm' },
                    });
                  }
                });
              }
  ...
  const connect = useCallback(
    (newRoomId: string, newUserId: string, newName: string) => {
      if (wsRef.current) {
        if (
      ...
      const ws = new WebSocket(
        `${wsUrl}?userId=${encodeURIComponent(newUserId)}&name=${encodeURIComponent(newName)}`
      );
      wsRef.current = ws;
          ...
          const camStream = await navigator.mediaDevices.getUserMedia({
            video: {
              deviceId: settings.selectedCameraId
                ? { exact: settings.selectedCameraId }
                : undefined,
              width: settings.hdVideo ? 1280 : 640,
              height: settings.hdVideo ? 720 : 480,
              frameRate: 30,
  ...
  const switchLayer = useCallback(
    (trackId: string, targetRid: string) => {
      sendSignalingMessage({
        type: 'layer-switch',
